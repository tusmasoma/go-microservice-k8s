...
func TestUseCase_ListCatalogItemsByNameContaining(t *testing.T) {
	t.Parallel()
	...
	patterns := []struct {
	...
	}{
		{
			name: "success",
			setup: func(tr *mock.MockCatalogItemRepository) {
				tr.EXPECT().ListByNameContaining(gomock.Any(), "apple").Return(
					[]entity.CatalogItem{
						item1,
						item2,
					},
		...
		},
		{
			...
			setup: func(tr *mock.MockCatalogItemRepository) {
				tr.EXPECT().ListByNameContaining(gomock.Any(), "docomo").Return(
					[]entity.CatalogItem{},
					nil,
				)
			},
	...
	for _, tt := range patterns {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			getCatalogItems, err := tuc.ListCatalogItemsByNameContaining(tt.arg.ctx, tt.arg.name)

			if (err != nil) != (tt.want.err != nil) {
				t.Errorf("ListCatalogItemsByNameContaining() error = %v, wantErr %v", err, tt.want.err)
			} else if err != nil && tt.want.err != nil && err.Error() != tt.want.err.Error() {
				t.Errorf("ListCatalogItemsByNameContaining() error = %v, wantErr %v", err, tt.want.err)
			}

			if !reflect.DeepEqual(getCatalogItems, tt.want.items) {
				t.Errorf("ListCatalogItemsByNameContaining() got = %v, want %v", getCatalogItems, tt.want.items)
			}
		})
	}
}
